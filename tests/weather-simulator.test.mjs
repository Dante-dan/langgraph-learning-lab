...
test("missing location exits through ask_user with a recoverable response", () => {
  const result = simulateTrip({ ...baseInput, hasLocation: false });

  assert.equal(result.status, "needs_input");
  assert.deepEqual(result.path.slice(-2), ["ask_user", "__end__"]);
  assert.deepEqual(result.response, { status: "needs_input", missing: ["location"] });
});
